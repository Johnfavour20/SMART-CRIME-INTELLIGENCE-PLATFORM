  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-8 py-8 flex flex-col gap-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        <div>
          <div className="flex items-center gap-2 text-[12px] font-semibold text-purple-700">
            ...
            <span>•</span>
            <span className="text-slate-500">Foundation Bible v1.0</span>
          </div>
        ...
        <div className="flex items-center gap-3">
          {onOpenExportTokens && (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-purple-700 text-white hover:bg-purple-800 text-[13px] font-semibold transition-all shadow-xs"
            >
      ...
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-[13px] text-slate-600">
            High-contrast clean light surfaces anchored with regal deep-violet primary tokens.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {colorTokens.map((c) => (
            ...
            >
              <div className={`w-full h-20 rounded-xl ${c.class} ${c.text} flex items-end p-2.5 font-mono text-[12px] font-bold ${c.border ? 'border border-slate-200' : ''}`}>
                {c.hex}
              </div>
              <div>
                ...
                <span className="text-[12px] text-slate-500 leading-tight block mt-0.5">
                  {c.role}
                </span>
              </div>
            </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="rounded-2xl bg-white border border-slate-200 divide-y divide-slate-200/80 overflow-hidden shadow-xs">
          {typeScale.map((t) => (
            <div key={t.role} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div className="min-w-[180px]">
                <span className="text-[13px] font-bold text-purple-700 block">{t.role}</span>
                <span className="text-[11px] text-slate-500">{t.size} • {t.weight}</span>
              </div>
              <div className="flex-1 text-slate-900 truncate font-medium" style={{ fontSize: t.size }}>
                {t.sample}
              </div>
            </div>
          ))}
